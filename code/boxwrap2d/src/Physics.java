...
public class Physics {
	...
	public Body createRect(float x0, float y0, float x1, float y1,int categorybits, int maskbits) {
        float cxs = (x0 + x1) * 0.5F;
        float cys = (y0 + y1) * 0.5F;
        float wxs = Math.abs(x1 - x0);
        float wys = Math.abs(y1 - y0);
        Vec2 center = this.m_draw.screenToWorld(cxs, cys);
        float halfWidthWorld = 0.5F * this.m_draw.screenToWorld(wxs);
        float halfHeightWorld = 0.5F * this.m_draw.screenToWorld(wys);
        PolygonDef pd = new PolygonDef();
	pd.filter.categoryBits = categorybits;
	pd.filter.maskBits = maskbits;
        pd.setAsBox(halfWidthWorld, halfHeightWorld);
